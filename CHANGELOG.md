...
### Changed
- Documented the scope-limiting rationale for standard-aligned outputs: a new "Why align: standard sidecars over bespoke integrations" section in [docs/schemas.md](docs/schemas.md) and a lean scope note in `AGENTS.md` explaining that we do not integrate telemetry backends, and that OpenTelemetry-aligned outputs let adopters publish/analyze via a standard sidecar (e.g. Grafana Alloy) instead of bespoke local integrations.

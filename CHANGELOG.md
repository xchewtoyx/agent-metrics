...
### Added
- Contract/invariant test suite (`tests/test_invariants.py`) that asserts documented guarantees rather than chasing coverage: single-source version, graceful degradation across the whole `OSError`/`SubprocessError` family, strict/deterministic JSON round-trip, the documented envelope shape, and that public/doc symbol references resolve.

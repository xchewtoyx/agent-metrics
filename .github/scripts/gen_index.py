...
Environment variables (GH_TOKEN or GITHUB_TOKEN must be set):
  ...
  GH_REPO       - owner/repo slug (e.g. xchewtoyx/agent-metrics)
...
def _api(path: str, token: str) -> Any:
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "agent-metrics/gen_index.py",
        },
    )
...
def _package_index_html(links: list[tuple[str, str, str]]) -> str:
    ...
    entries = "\n".join(
        ...
        for name, url, sha256 in sorted(
            links, key=lambda t: (_asset_version(t[0]), t[0])
        )
    )
    return (
        "<!DOCTYPE html>\n"
...
def _collect_links(
    releases: list[dict], hashes: dict[str, str]
) -> list[tuple[str, str, str]]:
    """Collect (name, url, sha256) for package assets in published releases."""
    ...
    for release in releases:
        ...
        for asset in release.get("assets", []):
            ...
            if not _is_package_asset(name):
                continue
            links.append((name, asset["browser_download_url"], hashes.get(name, "")))
    return links
...
def main(dist_dir: str, output_dir: str) -> None:
    ...
    links = _collect_links(_get_all_releases(repo, token), hashes)
